...
{
    [TestFixture]
    public class XiuyangshengxiTest
    {
        [Test]
        public async Task XiuyangshengxiTest_Success()
        {
            ...
            var xiuyang = new Xiuyangshengxi().AttachPlayerContext(new PlayerContext()
                {Player = player1, GameLevel = gameLevel1});
            player1.CardsInHand.Add(cardToPlay);
            player1.CardsInHand.Add(xiuyang);
            ...
            var response1 = await xiuyang.PlayCard(new CardRequestContext(), player1.RoundContext);
